...
baseline_tcp_bw_raw="$(qperf -lp "$SERVER_PORT" "$SERVER_IP" tcp_bw 2>/dev/null || true)"
baseline_tcp_lat_raw="$(qperf -lp "$SERVER_PORT" "$SERVER_IP" tcp_lat 2>/dev/null || true)"
...
echo "[cheaplanwatch] host=$(hostname -s) server=$SERVER_IP:$SERVER_PORT duration=${DURATION}s interval=${INTERVAL}s" | tee "$OUT_LOG"

tmp_samples="${OUT_JSON}.samples"
: > "$tmp_samples"
while [ "$(date +%s)" -lt "$end_ts" ]; do
  ts="$(date +%s)"
  tbw="$(qperf -lp "$SERVER_PORT" "$SERVER_IP" tcp_bw 2>/dev/null | qnum || true)"
  tlat="$(qperf -lp "$SERVER_PORT" "$SERVER_IP" tcp_lat 2>/dev/null | qnum || true)"
  ...
  printf '{"ts":%s,"tcp_bw":"%s","tcp_lat":"%s","ping_avg":"%s","ping_jitter":"%s"}\n' "$ts" "$tbw" "$tlat" "$pavg" "$pjit" >> "$tmp_samples"
  line="ts=${ts} tcp_bw=${tbw:-n/a} tcp_lat=${tlat:-n/a} ping_avg=${pavg:-n/a} ping_jitter=${pjit:-n/a}"
  echo "$line" | tee -a "$OUT_LOG"
  if [ -w /dev/tty1 ]; then echo "$line" > /dev/tty1 || true; fi
  sleep "$INTERVAL"
done

python3 - <<'PY' "$tmp_samples" "$OUT_JSON" "$baseline_tcp_bw" "$baseline_tcp_lat" "$base_ping_avg" "$base_ping_jitter" "$mtu_status"
import json, sys, pathlib
samples_path, out_json, b_bw, b_lat, b_avg, b_jit, mtu = sys.argv[1:]
samples=[]
...
echo "[cheaplanwatch] done: $OUT_JSON" | tee -a "$OUT_LOG"
