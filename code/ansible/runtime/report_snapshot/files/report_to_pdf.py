...
PINGPARTNER_DEFAULTS_PATH = (
    Path(__file__).resolve().parent.parent / "../bootstrapvm/roles/dhcp/defaults/main.yml"
).resolve()
...
def load_pingpartner_results(path: Optional[Path] = None) -> dict[str, str]:
...
def build_host_data(
    entries: Sequence[dict], pingpartner_results: Optional[dict[str, str]] = None
) -> List[dict]:
    ...
    for entry in entries:
        ...
        pingpartner_status = {
            partner: pingpartner_results.get(partner)
            for partner in ping_partners
            if pingpartner_results.get(partner)
        }
        hosts.append(
            {
                "host": host,
...
def load_icon_data_uri() -> Optional[str]:
    icon_path = Path(__file__).with_name("risng_icon.png")
    if icon_path.exists():
...
def render_html(
    hosts: Sequence[dict], metadata: dict, output_path: Path, icon_data_uri: Optional[str]
) -> None:
    total = metadata.get("total", len(hosts))
    reachable = metadata.get("reachable", 0)
    unreachable = metadata.get("unreachable", max(0, total - reachable))
...
def main(argv: Sequence[str]) -> int:
    ...
    render_html(hosts, metadata, html_path, icon_data_uri)
